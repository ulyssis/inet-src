...
void ManetRoutingBase::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj)
{
    if (signalID == mobilityStateChangedSignal)
    {
        ...
        yPosition = pos.y;
        posTimer = simTime();
		positionUpdated(xPosition,yPosition);
    }
}
...
double ManetRoutingBase::getXPos()
{
	if(xPosition<0)
	{
	    Coord currPos=check_and_cast<IMobility*>(getParentModule()->getSubmodule("mobility"))->getCurrentPosition();
        xPositionPrev = currPos.x;
        yPositionPrev = currPos.y;
        posTimerPrev = 0;
		xPosition=currPos.x;
		yPosition=currPos.y;
        posTimer = simTime();
