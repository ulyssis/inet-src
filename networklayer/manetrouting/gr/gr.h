...
typedef std::tr1::unordered_map<IPv4Address, GrNeighborInfo*, hashIPv4Address> nh_t;

class GR : public ManetRoutingBase
{
	...
	private:
		...
		struct GrNodeInfo
		{
			...
			const double x;
			const double y;
			GrNodeInfo(const IPv4Address& ip,double x,double y) : ip(ip),x(x),y(y) {}
		};

		double calculateLocalLinkQuality(const GrNodeInfo& src, const GrNodeInfo& a, const GrNodeInfo& self, const GrNodeInfo& b, const GrNodeInfo& dst) const;

		friend std::ostream& operator<<(std::ostream& os, const GR::GrNodeInfo& b);
	...
	public:
		...
		void updateLocalLinks(const GrNodeInfo& src, const GrNodeInfo& dst);
...
std::ostream& operator<<(std::ostream& os, const GR::GrNodeInfo& b)
    {
	os << b.ip << "(" << b.x << "," << b.y << ")";
	return os;
    }
